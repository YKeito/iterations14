...
#allRNASeq <- read.table("~/Nakano_RNAseq/network_analysis/base/CY_base/allRNASeq_union.txt", , sep = "\t", header = T, quote = "", row.names = 1, stringsAsFactors = F)
#genesymbol <- read.table("~/Nakano_RNAseq/network_analysis/base/CY_base/allRNASeq_genesymbol.txt", sep = "\t", header = F, quote = "", row.names = 1, stringsAsFactors = F)
temp <- genesymbol$V2[match(rownames(genesymbol), rownames(allRNASeq))]
allRNASeq <- data.frame(allRNASeq[, 1:36], 
                        gene_symbol = temp,
                        stringsAsFactors = F
                        )

AGI <- c("AT5G09410", "AT1G05830", "AT1G30490", "AT5G35210")
data <- allRNASeq[match(AGI, rownames(allRNASeq)), ]
...
for(i in i:total){
  ...
  plot(g)
  Fig_title <- paste0("~/Nakano_RNAseq/network_analysis/results_Fig/check_sub2/", title, ".png")
