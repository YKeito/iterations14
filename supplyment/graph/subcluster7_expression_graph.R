...
library(ggplot2)
library(reshape2)
subcluster7_AGI <- iterations14[iterations14$X__mclCluster == 7, ]$name
data <- allRNASeq[match(subcluster7_AGI, rownames(allRNASeq)), ]
col <- c("#F8766D", "a", "b", "c", "d", "#7CAE00", "e", "f", "g", "h", "#00BFC4")
total <- length(subcluster7_AGI)
i <- 1
for(i in i:total){
  ...
  plot(g)
  Fig_title <- paste0("~/Nakano_RNAseq/network_analysis/results_Fig/subcluster7/", title, ".png")
